...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if pygame.K_1 <= event.key <= pygame.K_8:
                current_tool = event.key - pygame.K_1  # Конвертируем клавишу в номер инструмента 0-7
        ...
        if event.type == pygame.MOUSEBUTTONUP and event.button == 1 and drawing:
            if current_tool == 1:  # Прямоугольник
                ...
                draw_equilateral_triangle(canvas, current_color, start_pos, mouse_pos, radius)
    ...
    tools = [
        "1-Карандаш", "2-Прямоуг.", "3-Круг", "4-Ластик",
        "5-Квадрат", "6-Треуг. прямоуг.", "7-Треуг. равност.", "8-Ромб"
    ]
    info = f"Инструмент: {tools[current_tool]} | Цвет: {current_color} (Пробел - сменить)"
